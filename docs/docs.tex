...
\begin{itemize}
\item \texttt{connectors.backoff\_num} - the number of times a client will try various Hive Connectors requests in case of an error. Has to be \textbf{a non-negative integer} greater than zero. Defaults to 1.
\item \texttt{connectors.backoff\_time} - the maximal backoff timeout in \textbf{milliseconds}. Has to be \textbf{a non-negative integer}. The concrete backoff timeout will be uniformly selected in the range [0, \texttt{connectors.backoff\_time} ]. Defaults to 0.
\end{itemize}
