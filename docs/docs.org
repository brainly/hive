...
- =hive.allowed_origins= - a JavaScript array of the origins that are allowed to access Hive services. Has to be *an array of strings*, each of which names a single origin - *a URL* (possibly with wildcards) or the special value "null". Some examples:
...
- =clients.hooks.cp.get= - the total number of =cp.get= Hook invocations.

- =clients.hooks.cp.put= - the total number of =cp.put= Hook invocations.

- =clients.hooks.cp.post= - the total number of =cp.post= Hook invocations.

#+latex: \noindent
Additionally, the same set of counters is defined on a per-hook-event basis. For an event named *$(name)*, the folowing counters will be added to the Monitor. Due to the dynamic nature of the hooks and their per-client character, *counters described below are added on use* and may not be included in the Monitor output at all times:

- =clients.hooks.$(name).cp.get= - the total number of =cp.get= Hook invocations.

- =clients.hooks.$(name).cp.put= - the total number of =cp.put= Hook invocations.

- =clients.hooks.$(name).cp.post= - the total number of =cp.post= Hook invocations.
...
- =cp_error= - Hive Protocol Hook related error, usually signalizes a problem with the backend that Hive talks to.
