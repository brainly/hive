...
#+latex: \noindent
Optional parameters:
- =connectors.backoff_num= - the number of times a client will try various Hive Connectors requests in case of an error. Has to be *a non-negative integer* greater than zero. Defaults to 1.

- =connectors.backoff_time= - the maximal backoff timeout in *milliseconds*. Has to be *a non-negative integer*. The concrete backoff timeout will be uniformly selected in the range [0, =connectors.backoff_time= ]. Defaults to 0.
